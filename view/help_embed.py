...
class HelpEmbed(discord.Embed):
    def __init__(self):
        ...
        self.add_field(name="Server Configuration", 
                       value="Use `/sconfig` to configure the server settings for LeetCode problems and contests. These commands are for Server Admins only. \n"
        ...
        self.add_field(name="Problem Configuration",
                       value="One of the main features that this bot supplies is providing reoccuring problems based on specified settings. Use `/pconfig <problemID>` to configure the server's problem settings. The problem ID is simply an identifier to differentiate the problems and their unique settings. If a problem exists, then it will be sent. To remove a problem use `/delproblem`. These commands are for Server Admins only. \n\n"
        ...
        self.add_field(name="User Commands",
                       ...
                       "If a user wishes to delete their profile, they can use `/deluser`.", inline=False)
